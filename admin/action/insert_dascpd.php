...
if(isset($_POST['submit'])){
		...
		$name_of_deceased = strtoupper(strtolower($_POST["name_of_deceased"]));
		...
		$stmt->close();
			...
			// Insert to client_death_aid_details Table
			$stmt = $con->prepare("INSERT INTO client_death_aid_details ( client_id, worker_id, name_of_deceased, category, interview_id ) VALUES (?,?,?,?,?)");
			$stmt->bind_param("iissi", $client_id, $worker_id, $name_of_deceased, $category, $interview_id);
			if(!$stmt->execute()){ $con->rollback(); }
			$stmt->close();
			...
			$sql = " INSERT INTO activity_log ( activity_date, details )
						 VALUES ( now() , ' $worker_name registered New Client named $firstname $lastname in Death Aid assistance of Senior Citizen & Person with Disablity' )";
			...
			if($con->multi_query($sql)){
				...
				echo '<script>alert("Client Unsuccessfully Registered");location="../forms/form-dascpd.php";</script>';
			}
...
// Insertion of Household ID
		...
		elseif ($family_head == $household['family_head'] AND $barangay == $household['barangay']) {
			...
			// Rollback the transaction
			$con->rollback();
			echo '<script>alert("ERROR! Contact the ADMIN");location="../forms/form-dascpd.php";</script>'; // Error if the condition does not met
		}
