<?php 
	...
	include "../../db_connect.php";
		...
		$interview_id = $_GET["interview_id"];
			...
			// Delete client referral details
			$sql = " DELETE FROM client_refferal_details where client_id = $client_id && client_refferal_detail_id = $count_no";
			
		if($con->multi_query($sql)){ //1
			 
			// Delete client assistance details
			$sql = " DELETE FROM assistance where client_id = $client_id && service_id = 12 && assistance_id = $assistance_id";
		
		if($con->multi_query($sql)){ //2
			 
			// Delete interview log
			$sql = " DELETE FROM interview_log where client_id = $client_id  && service_id = 12 && interview_id = $interview_id";
		...
		if($con->multi_query($sql)){ //4
			...
			VALUES ( now() , ' $worker_name DELETED $firstname $lastname details on Referral letter on other Agency' )";
