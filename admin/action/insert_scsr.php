...
if(isset($_POST['submit'])){
		...
		$stmt->close();
			...
			if($con->multi_query($sql)){
				...
				echo '<script>alert("Client Unsuccessfully Registered");location="../forms/form-scsr.php";</script>';
			}
...
// Insertion of Household ID
		...
		elseif ($family_head == $household['family_head'] AND $barangay == $household['barangay']) {
			...
			// Rollback the transaction
			$con->rollback();
			echo '<script>alert("ERROR! Contact the ADMIN");location="../forms/form-scsr.php";</script>'; // Error if the condition does not met
		}
