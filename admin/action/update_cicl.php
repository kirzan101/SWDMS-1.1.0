...
if(isset($_POST['submit'])){
		...
		$stmt->close();
			...
			// Update to client_child_details table
			$stmt = $con->prepare("UPDATE client_child_details SET reffered_by = ?, type_of_offense = ?, committed = ?, when_happened = ?, where_happned = ?, to_whom = ?, services_rendered, disposition = ? WHERE interview_id = ?");
			$stmt->bind_param("ssssssssi", $refer, $type, $committed, $when, $where, $whom, $service, $disposition, $interview_id);
			if(!$stmt->execute()){ $con->rollback(); }
			$stmt->close();
			
			// Insert to activity_log Table 
			$sql = " INSERT INTO activity_log ( activity_date, details )
						 VALUES ( now() , ' $worker_name Updated the Client named $firstname $lastname details of Children in Conflict with the Law' )";
