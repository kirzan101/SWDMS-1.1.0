<?php 
	...
	include "../../db_connect.php";
		...
		if($con->multi_query($sql)){ //1
			...
			$sql = " DELETE FROM assistance where client_id = $client_id && service_id = 2 && assistance_id = $assistance_id";
		
		if($con->multi_query($sql)){ //2
			 
			// Delete Interview log
			$sql = " DELETE FROM interview_log where client_id = $client_id  && service_id = 2 && interview_id = $interview_id";
		...
		VALUES ( now() , ' $worker_name DELETED $firstname $lastname details on AICS - Educational Assistance' )";
