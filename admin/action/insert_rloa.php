...
if(isset($_POST['submit'])){
		...
		// Service inputs
		$referral_details = ucfirst(strtolower($_POST["referral_details"]));
		$referral_letter_details = ucfirst(strtolower($_POST["referral_letter_details"]));
		...
		$stmt->close();
			...
			// Insert to client_referral_details Table
			$stmt = $con->prepare("INSERT INTO client_referral_details ( client_id, referral_details, referral_letter_details, interview_id ) VALUES (?,?,?,?)");
			$stmt->bind_param("issi", $client_id, $referral_details, $referral_letter_details, $interview_id);
			if(!$stmt->execute()){ $con->rollback(); }
			$stmt->close();
			...
			$sql = " INSERT INTO activity_log ( activity_date, details )
						 VALUES ( now() , ' $worker_name registered New Client named $firstname $lastname in Referral letter to other Agency' )";
			...
			if($con->multi_query($sql)){
				...
				echo '<script>alert("Client Unsuccessfully Registered");location="../forms/form-rloa.php";</script>';
			}
...
// Insertion of Household ID
		...
		elseif ($family_head == $household['family_head'] AND $barangay == $household['barangay']) {
			...
			// Rollback the transaction
			$con->rollback();
			echo '<script>alert("ERROR! Contact the ADMIN");location="../forms/form-rloa.php";</script>'; // Error if the condition does not met
		}
