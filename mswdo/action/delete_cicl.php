<?php 
	...
	include "../../db_connect.php";
		...
		$interview_id = $_GET["interview_id"];
			...
			$sql = " DELETE FROM client_child_details where client_id = $client_id && client_child_detail_id = $count_no ";
			
		if($con->multi_query($sql)){ //1
			 
			// Delete client assistance details
			$sql = " DELETE FROM assistance where client_id = $client_id && service_id = 6 && assistance_id = $assistance_id";
		
		if($con->multi_query($sql)){ //2
			 
			// Delete interview log
			$sql = " DELETE FROM interview_log where client_id = $client_id  && service_id = 6 && interview_id = $interview_id";
		...
		VALUES ( now() , ' $worker_name DELETED $firstname $lastname details on Children in Conflict with the Law' )";
