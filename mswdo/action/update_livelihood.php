...
if(isset($_POST['submit'])){
		...
		$stmt->close();
			...
			// Update to client_training_details table
			$stmt = $con->prepare("UPDATE client_training_details SET training_needs_1 = ?, training_needs_2 = ?, training_needs_3 = ?, startup_kit = ? WHERE interview_id = ?");
			$stmt->bind_param("ssssi", $choice1, $choice2, $choice3, $startup, $interview_id);
			if(!$stmt->execute()){ $con->rollback(); }
			$stmt->close();
			
			// Insert to activity_log Table 
			$sql = " INSERT INTO activity_log ( activity_date, details )
						 VALUES ( now() , ' $worker_name Updated the Client named $firstname $lastname details of Livelihood Assistance ' )";
			...
			if($con->multi_query($sql)){
				...
				echo '<script>alert("Client Unsuccessfully Registered");location="../update/update-liveihood.php";</script>';
			}
