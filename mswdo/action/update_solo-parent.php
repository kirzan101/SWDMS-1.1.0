...
if(isset($_POST['submit'])){
		...
		$stmt->close();
			...
			// Update to client_solo_details table
			$stmt = $con->prepare("UPDATE client_scsr_details SET income_monthly = ?, classification = ?, needs = ?, fam_resources = ? WHERE interview_id = ?");
			$stmt->bind_param("ssssi",$incomem, $needs, $classification, $resources, $interview_id);
			if(!$stmt->execute()){ $con->rollback(); }
			$stmt->close();
			
			// Insert to activity_log Table 
			$sql = " INSERT INTO activity_log ( activity_date, details )
						 VALUES ( now() , ' $worker_name Updated the Client named $firstname $lastname details of Issuance of Solop Parent and Person with Disability ID ' )";
			...
			if($con->multi_query($sql)){
				...
				echo '<script>alert("Client Unsuccessfully Registered");location="../update/update-solo-parent.php";</script>';
			}
