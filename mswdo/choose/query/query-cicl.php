<?php
	...
	// Query for CICL services
	$sql = "SELECT * FROM client c LEFT JOIN client_general_details g ON c.client_id = g.client_id LEFT JOIN assistance a ON c.client_id = a.client_id LEFT JOIN client_child_details d ON c.client_id = d.client_id 
	...
	// client_child_details Table
	$refer = $row['reffered_by'];
	$type = $row['type_of_offense'];
	$committed = $row['committed'];
	$when = $row['when_happened'];
	$where = $row['where_happened'];
	$whom = $row['to_whom'];
	$service = $row['services_renderd'];
