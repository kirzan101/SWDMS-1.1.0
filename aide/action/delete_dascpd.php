<?php 
	...
	include "../../db_connect.php";
		...
		$interview_id = $_GET["interview_id"];
			...
			// Delete client death aid details
			$sql = " DELETE FROM client_death_aid_details where client_id = $client_id && client_death_aid_detail_id = $count_no ";
			
		if($con->multi_query($sql)){ //1
			 
			// Delete client assistance
			$sql = " DELETE FROM assistance where client_id = $client_id && service_id = 10 && assistance_id = $assistance_id";
		
		if($con->multi_query($sql)){ //2
			 
			// Delete client interview log
			$sql = " DELETE FROM interview_log where client_id = $client_id  && service_id = 10 && interview_id = $interview_id";
			
		if($con->multi_query($sql)){ //3
			 
			// Delete client intervew details
		...
		if($con->multi_query($sql)){ //4
			...
			VALUES ( now() , ' $worker_name DELETED $firstname $lastname details on Death Aid for Senior Citizen an Person with Disability' )";
