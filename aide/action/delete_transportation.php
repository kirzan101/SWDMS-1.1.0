<?php 
	...
	include "../../db_connect.php";
		...
		$interview_id = $_GET["interview_id"];
			...
			$sql = " DELETE FROM client_general_details where client_id = $client_id && client_general_detail_id";
			
		if($con->multi_query($sql)){ //1
			 
			// Delete client assistance details
			$sql = " DELETE FROM assistance where client_id = $client_id && service_id = 5 && assistance_id = $assistance_id";
		
		if($con->multi_query($sql)){ //2
			 
			// Delete interview_log
			$sql = " DELETE FROM interview_log where client_id = $client_id  && service_id = 5 && interview_id = $interview_id";
		...
		VALUES ( now() , ' $worker_name DELETED $firstname $lastname details on AICS - Transportation Assistance' )";
