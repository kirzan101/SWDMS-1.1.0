<?php 
	...
	include "../../db_connect.php";
		...
		$con->query('START TRANSACTION;');
			...
			$sql = " UPDATE client SET `status` = 'ACTIVE' where client_id = $client_id";
			
		if($con->multi_query($sql)){ //1
			 
		// Insert to activity_log Table 
		$sql = " INSERT INTO activity_log ( activity_date, details )
		VALUES ( now() , ' $worker_name RESTORE $firstname $lastname records' )";

		if($con->multi_query($sql)){
				echo '<script>alert("Client Successfully Restore");location="../table/index.php";</script>';
			}
			else{
				// Rollback the transaction
				$con->rollback();
				echo '<script>alert("Client Unsuccessfully Restore");location="../table/delete-index.php?client_id='.$client_id.'";</script>';
			}
