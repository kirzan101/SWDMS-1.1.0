<?php
	...
	if(isset($_POST['submit'])){
		...
		$stmt->bind_param("sssssi", $educm, $contactnom, $civilm, $incomem, $skillm, $client_id);
		if(!$stmt->execute()){ $con->rollback(); $err = 1; }
		$husband_id = $client_id;
		$stmt->close();
			...
			$stmt = $con->prepare("INSERT INTO interview_log ( client_id, worker_id, service_id, interview_id ) VALUES (?,?,?,?)");
			$stmt->bind_param("iiii", $wife_id, $worker_id, $service_id, $interview_idf);
			if(!$stmt->execute()){ $con->rollback(); $err = 2; }
			$stmt->close();
			...
			$sql = " INSERT INTO activity_log ( activity_date, details )
						 VALUES ( now() , ' $worker_name registered New couple named $firstnamem $lastname and $firstnamef $lastnamef in Premarriage Counselling' )";
			...
			if($con->multi_query($sql) AND $err == 0 ){
				...
				echo '<script>alert("Client Unsuccessfully Registered");location="../choose/choose-pre-marriage.php?client_id='.$husband_id.'";</script>';
			}	
		...
		$stmt->close();
			...
			// Insert to client table
			$stmt = $con->prepare("UPDATE client SET educ_attainment = ?, contact_no = ?, civil_status = ?, income = ?, skills = ? WHERE client_id= ?");
			$stmt->bind_param("sssssi", $educf, $contactnof, $civilf, $incomef, $skillf, $client_id);
			if(!$stmt->execute()){ $con->rollback(); $err = 1; }
			$wife_id = $client_id;
			$stmt->close();
			...
			// Insert to client_premarriage_details Table 
			$stmt = $con->prepare("INSERT INTO client_premarriage_details(client_id, address_marriage, interview_id, income_monthly) VALUES (?,?,?,?)");
			$stmt->bind_param("isis", $wife_id, $afaddf, $interview_idf, $mincomef);
			if(!$stmt->execute()){ $con->rollback(); $err = 1; }
			$stmt->close();	
			...
			if($con->multi_query($sql) AND $err == 0 ){
				...
				$con->rollback();
				// Notification if one or more data is not successfully inserted
				echo '<script>alert("Client Unsuccessfully Registered");location="../choose/choose-pre-marriage.php?client_id='.$wife_id.'";</script>';
			}
				
		}
		// Insertion of Household ID
		//'.$wife_id.','.$worker_id.','.$service_id.','.$interview_idf.'
		// Query for Barangay
		...
		elseif ($family_headm == $householdm['family_head'] AND $barangaym == $householdm['barangay']) {
			...
			// Rollback the transaction
			$con->rollback();
			echo '<script>alert("ERROR! Contact the ADMIN");location="../choose/choose-pre-marriage.php?client_id='.$husband_id.'";</script>'; // Error if the condition does not met
		}
		...
		elseif ($family_headf == $householdf['family_head'] AND $barangayf == $householdf['barangay']) {
			...
			// Rollback the transaction
			$con->rollback();
			echo '<script>alert("ERROR! Contact the ADMIN");location="../choose/choose-pre-marriage.php?client_id='.$wife_id.'";</script>'; // Error if the condition does not met
		}
