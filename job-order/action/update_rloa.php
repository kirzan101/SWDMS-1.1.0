...
if(isset($_POST['submit'])){
		...
		$stmt->close();
			...
			// Update to client_referral_details table
			$stmt = $con->prepare("UPDATE client_referral_details SET referral_details = ?, referral_letter_details = ? WHERE interview_id = ?");
			$stmt->bind_param("ssi", $referral_details, $referral_letter_details, $interview_id);
			if(!$stmt->execute()){ $con->rollback(); }
			$stmt->close();
			
			// Insert to activity_log Table 
			$sql = " INSERT INTO activity_log ( activity_date, details )
						 VALUES ( now() , ' $worker_name Updated the Client named $firstname $lastname details of Referral Letter to other Agency ' )";
			...
			if($con->multi_query($sql)){
				...
				echo '<script>alert("Client Unsuccessfully Registered");location="../update/update-rloa.php";</script>';
			}
