<?php 
	...
	include "../../db_connect.php";
		...
		$interview_id = $_GET["interview_id"];
			...
			// Delete Disadvantage Woman, Youth, and Other Needy Adult Details
			$sql = " DELETE FROM client_dwyna_details where client_id = $client_id && client_dwyna_detail_id = $count_no";
			
		if($con->multi_query($sql)){ //1
			 
			// Delete Client Assistance Details
			$sql = " DELETE FROM assistance where client_id = $client_id && service_id = 7 && assistance_id = $assistance_id";
		
		if($con->multi_query($sql)){ //2
			 
			// Delete Interview log
			$sql = " DELETE FROM interview_log where client_id = $client_id  && service_id = 7 && interview_id = $interview_id";
		...
		if($con->multi_query($sql)){ //4
			...
			VALUES ( now() , ' $worker_name DELETED $firstname $lastname details on Disadvantaged Woman, Youth, and other Needy adult' )";
