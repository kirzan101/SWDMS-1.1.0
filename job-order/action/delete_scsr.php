<?php 
	...
	include "../../db_connect.php";
		...
		$interview_id = $_GET["interview_id"];
			...
			// Delete Social case study report details 
			$sql = " DELETE FROM client_scsr_details where client_id = $client_id && client_scsr_detail_id = $count_no";
			
		if($con->multi_query($sql)){ //1
			 
			// Delete client assistant details
			$sql = " DELETE FROM assistance where client_id = $client_id && service_id = 13 && assistance_id = $assistance_id";
		
		if($con->multi_query($sql)){ //2
			 
			// Delete interview log
			$sql = " DELETE FROM interview_log where client_id = $client_id  && service_id = 13 && interview_id = $interview_id";
		...
		if($con->multi_query($sql)){ //4
			...
			VALUES ( now() , ' $worker_name DELETED $firstname $lastname details on Social Case Study Report' )";
