...
if(isset($_POST['submit'])){
		...
		// Service inputs
		$details = 'Person with Disability ID';
		...
		$stmt->close();
			...
			// Insert to client_pwd_solo Table
			$stmt = $con->prepare("INSERT INTO client_pwd_solo (client_id, details, interview_id)  VALUES (?,?,?)");
			$stmt->bind_param("isi", $client_id, $details, $interview_id);
			if(!$stmt->execute()){ $con->rollback(); }
			$stmt->close();
			...
			$sql = " INSERT INTO activity_log ( activity_date, details )
						 VALUES ( now() , ' $worker_name registered Existing Client named $firstname $lastname in Issuance of Solo Parent and Person with Disability ID' )";
			...
			if($con->multi_query($sql)){
				...
				echo '<script>alert("Client Unsuccessfully Registered");location="../forms/form-pwd.php";</script>';
			}
