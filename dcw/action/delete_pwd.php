<?php 
	...
	include "../../db_connect.php";
		...
		$interview_id = $_GET["interview_id"];
			...
			// Delete Person with disability details
			$sql = " DELETE FROM client_pwd_solo where client_id = $client_id && pwd_solo_id = $count_no ";
			
		if($con->multi_query($sql)){ //1
			 
			// Delete client assistance details
			$sql = " DELETE FROM assistance where client_id = $client_id && service_id = 9 && assistance_id = $assistance_id";
		
		if($con->multi_query($sql)){ //2
			 
			//  Delete interview log
			$sql = " DELETE FROM interview_log where client_id = $client_id  && service_id = 9 && interview_id = $interview_id";
			
		if($con->multi_query($sql)){ //3
			 
			// Delete interview details
			$sql = " DELETE FROM interview where client_id = $client_id && interview_id";
		
		if($con->multi_query($sql)){ //4
			 
			// Insert to activity_log Table 
			$sql = " INSERT INTO activity_log ( activity_date, details )
			VALUES ( now() , ' $worker_name DELETED $firstname $lastname details on Issuance of Solo Parent & Person with Disability ID' )";
