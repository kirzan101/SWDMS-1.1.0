...
if(isset($_POST['submit'])){
		...
		$stmt->close();
			...
			// Update to client_dascpd_details table
			$stmt = $con->prepare("UPDATE client_death_aid_details SET category = ?,name_of_deceased = ? WHERE interview_id = ?");
			$stmt->bind_param("ssi", $category, $name_of_deceased, $interview_id);
			if(!$stmt->execute()){ $con->rollback(); }
			$stmt->close();
			
			// Update to assistance table
			$stmt = $con->prepare("UPDATE assistance SET amount = ? WHERE interview_id = ?");
			$stmt->bind_param("si", $amount, $interview_id);
			if(!$stmt->execute()){ $con->rollback(); }
			$stmt->close();
			
			// Insert to activity_log Table 
			$sql = " INSERT INTO activity_log ( activity_date, details )
						 VALUES ( now() , ' $worker_name Updated the Client named $firstname $lastname details of Death Aid for Senior Citizen and Person with Disability' )";
			...
			if($con->multi_query($sql)){
				...
				echo '<script>alert("Client Unsuccessfully Registered");location="../update/update-cicl.php";</script>';
			}
