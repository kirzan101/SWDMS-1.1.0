...
if(isset($_POST['submit'])){
		...
		$needs = ucfirst(strtolower($_POST["needs"]));
		$classification = ucfirst(strtolower($_POST["classification"]));
		$resources = ucfirst(strtolower($_POST["resources"]));
		...
		$stmt->close();
			...
			// Insert to client_solo_parent_details Table
			$stmt = $con->prepare("INSERT INTO client_solo_details ( client_id, income_monthly, classification, needs, fam_resources, interview_id ) VALUES (?,?,?,?,?,?)");
			$stmt->bind_param("issssi", $client_id, $incomem, $classification, $needs, $resources, $interview_id);
			if(!$stmt->execute()){ $con->rollback(); }
			$stmt->close();
			...
			if($con->multi_query($sql)){
				...
				echo '<script>alert("Client Unsuccessfully Registered");location="../forms/form-solo-parent.php";</script>';
			}
...
// Insertion of Household ID
		...
		elseif ($family_head == $household['family_head'] AND $barangay == $household['barangay']) {
			...
			// Rollback the transaction
			$con->rollback();
			echo '<script>alert("ERROR! Contact the ADMIN");location="../forms/form-solo-parent.php";</script>'; // Error if the condition does not met
		}
