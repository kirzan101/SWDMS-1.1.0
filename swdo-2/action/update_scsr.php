...
if(isset($_POST['submit'])){
		...
		$stmt->close();
			...
			// Update to client_scsr_details table
			$stmt = $con->prepare("UPDATE client_scsr_details SET background_info = ?, recommendation = ? WHERE interview_id = ?");
			$stmt->bind_param("ssi", $background, $recommendation, $interview_id);
			if(!$stmt->execute()){ $con->rollback(); }
			$stmt->close();
			
			// Insert to activity_log Table 
			$sql = " INSERT INTO activity_log ( activity_date, details )
						 VALUES ( now() , ' $worker_name Updated the Client named $firstname $lastname details of Social Case Study Report' )";
			...
			if($con->multi_query($sql)){
				...
				echo '<script>alert("Client Unsuccessfully Registered");location="../update/update-scsr.php";</script>';
			}
