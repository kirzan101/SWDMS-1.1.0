<?php 
	...
	include "../../db_connect.php";
		...
		$interview_id = $_GET["interview_id"];
			...
			// Delete indigency Details
			$sql = " DELETE FROM client_indigency_details where client_id = $client_id && client_indigency_detail_id";
			
		if($con->multi_query($sql)){ //1
			 
			// Delete Client assistance details
			$sql = " DELETE FROM assistance where client_id = $client_id && service_id = 14 && assistance_id = $assistance_id";
		
		if($con->multi_query($sql)){ //2
			 
			// Delete interview log
			$sql = " DELETE FROM interview_log where client_id = $client_id  && service_id = 14 && interview_id = $interview_id";
		...
		VALUES ( now() , ' $worker_name DELETED $firstname $lastname details on Issuance of Certificate of Indigency' )";
