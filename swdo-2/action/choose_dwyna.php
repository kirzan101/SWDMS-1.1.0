...
if(isset($_POST['submit'])){
		...
		if(!$stmt->execute()){ $con->rollback(); $testErr = 1; }
		$stmt->close();
			...
			if(!$stmt->execute()){ $con->rollback(); $testErr = 2; }
			...
			if(!$stmt->execute()){ $con->rollback(); $testErr = 3; }
			$stmt->close();
			...
			if(!$stmt->execute()){ $con->rollback(); $testErr = 4; }
			$stmt->close();
			...
			if(!$stmt->execute()){ $con->rollback(); $testErr = 5; }
			$stmt->close();
			...
			echo '<script>alert("ERROR: Please Contact the Admin'.$testErr.'");location="../table/service_registry.php";</script>';
		}
