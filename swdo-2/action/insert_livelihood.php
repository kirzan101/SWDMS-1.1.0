...
if(isset($_POST['submit'])){
		...
		// Service inputs
		//$choice1 = if(empty($_POST["choice1"])) { echo 'N/A'; } else { echo ucfirst(strtolower($_POST["choice1"])); }
		//$choice2 = if(empty($_POST["choice2"])) { echo 'N/A'; } else { echo ucfirst(strtolower($_POST["choice2"])); }
		//$choice3 = if(empty($_POST["choice3"])) { echo 'N/A'; } else { echo ucfirst(strtolower($_POST["choice3"])); }
		//$startup = if(empty($_POST["startup"])) { echo 'N/A'; } else { echo ucfirst(strtolower($_POST["startup"])); }
		...
		$stmt->close();
			...
			// Insert to client_training_details Table
			$stmt = $con->prepare("INSERT INTO client_training_details ( client_id, training_needs_1, training_needs_2, training_needs_3, startup_kit, interview_id ) VALUES (?,?,?,?,?,?)");
			$stmt->bind_param("issssi", $client_id, $choice1, $choice2, $choice3, $startup, $interview_id);
			if(!$stmt->execute()){ $con->rollback(); }
			$stmt->close();
			...
			$sql = " INSERT INTO activity_log ( activity_date, details )
						 VALUES ( now() , ' $worker_name registered New Client named $firstname $lastname in Livelihood Assistance' )";
			...
			if($con->multi_query($sql)){
				...
				echo '<script>alert("Client Unsuccessfully Registered");location="../forms/form-livelihood.php";</script>';
			}
...
// Insertion of Household ID
		...
		elseif ($family_head == $household['family_head'] AND $barangay == $household['barangay']) {
			...
			// Rollback the transaction
			$con->rollback();
			echo '<script>alert("ERROR! Contact the ADMIN");location="../forms/form-livelihood.php";</script>'; // Error if the condition does not met
		}
