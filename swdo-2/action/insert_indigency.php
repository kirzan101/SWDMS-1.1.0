...
if(isset($_POST['submit'])){
		...
		$stmt->close();
			...
			// Insert to client_indigency_details Table
			$stmt = $con->prepare("INSERT INTO client_indigency_details ( client_id, indigency_letter_details , interview_id ) VALUES (?,?,?)");
			$stmt->bind_param("isi", $client_id, $background, $interview_id);
			if(!$stmt->execute()){ $con->rollback(); }
			$stmt->close();
			...
			$sql = " INSERT INTO activity_log ( activity_date, details )
						 VALUES ( now() , ' $worker_name registered New Client named $firstname $lastname in Securing Certificate of Indigency' )";
			...
			if($con->multi_query($sql)){
				...
				echo '<script>alert("Client Unsuccessfully Registered");location="../forms/form-indigency.php";</script>';
			}
...
// Insertion of Household ID
		...
		elseif ($family_head == $household['family_head'] AND $barangay == $household['barangay']) {
			...
			// Rollback the transaction
			$con->rollback();
			echo '<script>alert("ERROR! Contact the ADMIN");location="../forms/form-indigency.php";</script>'; // Error if the condition does not met
		}
